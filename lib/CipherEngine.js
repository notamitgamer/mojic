...
export class CipherEngine {
    ...
    async init(existingSaltHex = null, expectedAuthCheck = null) {
        ...
        if (expectedAuthCheck) {
            ...
            const calcAuthBuffer = Buffer.from(calculatedAuthCheck, 'hex');
            const expectedAuthBuffer = Buffer.from(expectedAuthCheck, 'hex');
            if (calcAuthBuffer.length !== expectedAuthBuffer.length || !crypto.timingSafeEqual(calcAuthBuffer, expectedAuthBuffer)) {
